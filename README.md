...
| branch | source commit | point in the trials |
|---|---|---|
| `main` | `d8a618d` (2026-07-21) | base control stack, before any trial (the WetRobo input) |
| `wetrobo+petri` | `3a1706b` (2026-08-06 11:03 JST) | end of the Petri dish lid trial |
| `wetrobo+petri+cap` | `1f07761` (2026-08-06 18:37 JST) | verified bottle cap transfer |
| `wetrobo+petri+cap+door` | `676981b` (2026-08-08 23:45 JST) | autonomous incubator door, tag-position-independent retargeting — the code exemplar |

### Running the cap transfer from `wetrobo+petri+cap`

The task profile `src/configs/pasteur_culture_media_cap_grasp.json` refers to
run data under `data/` (tap identity, head captures, scene reconstruction),
which is not tracked. `data` here is a symlink to the original repository's
`data/` directory. Entry point, as in `skills/run-cylindrical-cap-transfer/SKILL.md`:

```bash
cd ~/src/google/wetrobo        # branch wetrobo+petri+cap
/home/admin/miniforge3/envs/robot-test/bin/python src/run_culture_media_cap_grasp.py \
...
- `robot/camera_map.json` in this snapshot is `{head: 0, left: 1, right: 2}`.
  The host's current map (changed 2026-08-14) is `{head: 0, left: 2, right: 1}`.
  The head index is the same; if a wrist camera is used, copy the current map
  over this one first.
- A new bottle position needs a fresh tap: the profile's `tap_identity` points
  at the 2026-08-06 tap. The skill says a new position may reuse the adapter
  and gates but must reacquire its tap and plan a fresh approach.
- `robot/arm/calibrate_left.py` moves the left gripper on import; do not
  import it or run an import-all check over the tree.
